...
const samePattern = (str, arr) => {
  ...
  if (uniqueStr.length === uniqueArray.length) {
    arr.forEach((ele, index) => {
      if (arrCopy.indexOf(ele) == -1) {
        answer.push(uniqueStr[index]);
        arrCopy.push(uniqueArray[index]);
        // console.log("true", index)
      } else {
        let occurenceIndex = arrCopy.indexOf(ele);
        answer.push(uniqueStr[occurenceIndex]);
        arrCopy.push(uniqueArray[occurenceIndex]);
  ...
  if (JSON.stringify(answer) == JSON.stringify(str.split(""))) {
    return true;
  }
};
// -------------------------------------------------------------------------------------------------------
